...
model_list.update(
    {
        key: models.resnet50(weights=None, num_classes=value, args=None)
...
check_path = os.path.join("checkpoint", git_name, "class", name, "save_model")
...
class CustomDataset_class(Dataset):
    ...
    def load_list(self, mode="train"):
        ...
        self.json_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        self.json_dict_train = copy.deepcopy(self.json_dict)

        for equ_name in sub_path_list:
            if equ_name.startswith(".") or int(equ_name) not in [1]:
                continue
...
for key in model_list:
    # if key == "dryness": continue
    model = model_list[key].cuda()
    test_list = list()
    for w_key in model_area_dict[key]:
        test_list.append(dataset.load_dataset("test", w_key))

    test_list = [j for w_key_list in test_list for j in w_key_list]
    ...
    for idx, (img, grade, img_name, dig_, _) in tqdm(
        enumerate(testset_loader), desc=key
    ):
        img = img.cuda()
        model = model_list[key].cuda()
        model.eval()
        ...
        pil_imgs = np.array(img.permute(0, 2, 3, 1).detach().cpu())
        for i in range(len(grayscale_cams)):
            grayscale_cam = grayscale_cams[i, :]
            pil_img = np.array(pil_imgs[i, :] / pil_imgs[i, :].max())
            v_img.append(show_cam_on_image(pil_img, grayscale_cam, use_rgb=True))

        stacked_images = np.stack(v_img, axis=0)
        c_img = make_grid(
            torch.tensor(stacked_images).permute(0, 3, 1, 2), nrow=8
        ).numpy()

        cv2.imwrite(f"cam_output/GradCAM/{dig_[0]}_{idx}.jpg", c_img[(2, 1, 0), :, :].transpose(1, 2, 0))

    # %%
    # from torchvision.utils import make_grid

    # for m_dig, datalist in testset_loader.items():
    #     model = model_list[m_dig]
    #     model.eval()
    #     model = model.cuda()
    #     cam = GradCAM(model=model, target_layers=[model.layer4[-1]])
    ...
    #     for img, label, img_name, _, pil_imgs in tqdm(loader_datalist, desc=m_dig):
    #         img, label = img.cuda(), label.cuda()
    #         v_img = list()
    #         grayscale_cams = cam(input_tensor=img, targets=None)
    #         pred = [j.item() for j in torch.argmax(model(img), dim=1)]
    #         for i in range(len(grayscale_cams)):
    #             grayscale_cam = grayscale_cams[i, :]
    #             pil_img = np.array(pil_imgs[i, :] / pil_imgs[i, :].max())
    #             img_mat = cv2.UMat(
    #                 show_cam_on_image(pil_img[:, :, (2, 1, 0)], grayscale_cam, use_rgb=True)
    #             )
    #             img_mat = cv2.putText(
    #                 img_mat,
    #                 img_name[i],
    ...
    #                 cv2.LINE_AA,
    #             )
    #             v_img.append(img_mat.get())

    #         stacked_images = np.stack(v_img, axis=0)
    #         c_img = (
    #             make_grid(torch.tensor(stacked_images).permute(0, 3, 1, 2), nrow=8)
    #             .permute(1, 2, 0)
    #             .numpy()
    #         )
